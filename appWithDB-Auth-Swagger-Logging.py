...
logging.basicConfig(
    filename='/app/logs/app.log',  # Log file location inside the container
    level=logging.INFO,
...
class Register(Resource):
    ...
    def post(self):
        ...
        if not username or not password or not email:
            logger.error('Failed to get username, password and email')
...
class AddComment(Resource):
    ...
    def post(self):
        ...
        if not content or not user_id or not post_id:
            logger.error('content, user_id, and post_id are required')
...
class Users(Resource):
    def get(self):
        ...
        logger.info('All users detailes called')
...
class Posts(Resource):
    def get(self):
        ...
        logger.info('All posts details called')
...
class UserPost(Resource):
    def get(self, user_id):
        ...
        if not posts_data:
            logger.error('No posts available for this user id')
            return jsonify({'error': 'No posts available for this user id'})

        posts = [{'post_id': post[0], 'title': post[1], 'content': post[2], 'timestamp': post[3], 'user_id': post[4]} for post in posts_data]
        logger.info('For user %s post detailes called', user_id)
        return jsonify({'posts': posts})
...
class PostComment(Resource):
    def showComments(self, post_id):
        ...
        if not comments_data:
            logger.error('No comments available for post %s', post_id)
        ...
        logger.info('Comments displayed for post %s', post_id)
